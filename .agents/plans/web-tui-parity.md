...
### Milestone 2 — The visual language and a Markdown renderer that cannot inject markup

With the assets in real files, give the viewer the terminal's look and a safe renderer.

Write `src/web/viewer.css` as a token layer plus components. Declare `@font-face` for JetBrains Mono against `/fonts/jetbrains-mono.woff2` with `font-display: swap` and a real monospace fallback stack, and use it for the whole application: the terminal surface is monospace, and matching it is most of the resemblance. Define custom properties for the palette so the semantic names appear once. Map the terminal's colours as follows: cyan for the user role and for focus, yellow for the agent role and for running work, green for success and healthy headroom, red for failure and exhausted headroom, magenta for plans, and a dim grey for thinking and for system notes. Keep flat bordered surfaces with restrained corner radii rather than the current fourteen-pixel cards, and give every focusable element a visible focus border rather than relying on the browser default, which is invisible against a dark ground on some phones.

Add the role glyphs to the transcript: `❯` for You, `●` for Agent, `○` for Thinking, `◇` for Plan, `◈` for Proposed plan, `─` for Hel, and the four tool glyphs `• ● ✓ ×`. Take them from the same values `entry_visual` uses so the two surfaces cannot drift; Milestone 3 will carry the glyph and colour name in the projection so the browser does not hard-code a second copy.

Replace every place that builds markup from strings. The current code has `escapeHtml`, `escapeAttr` and several template literals assigned to `innerHTML`, including the whole session list. Delete all of them. Build DOM nodes with `document.createElement` and `textContent`, and write one Markdown renderer, in `src/web/markdown.js`, that returns a `DocumentFragment` and never touches `innerHTML`. Support ATX headings, ordered and unordered lists including nesting, fenced and inline code, links, emphasis and strong emphasis, block quotes, horizontal rules, and pipe tables. Render a link as an `<a>` whose `href` is accepted only when it parses as `http:`, `https:` or `mailto:`; anything else, including `javascript:` and `data:`, renders as plain text. Render a diff summary — the `tool_diffstats` lines the transcript already carries, of the form `path | +12 -3` — as its own element so it can be styled without being parsed as a table.

The hard rule for this milestone, and for the rest of the plan: agent-controlled content never reaches `innerHTML`, `outerHTML`, `insertAdjacentHTML`, `document.write`, or an `href`/`src` attribute that has not been scheme-checked. Add a test that greps `src/web/*.js` for those sinks and fails if any appears outside an allowlist, so a later contributor cannot reintroduce one quietly.

You will know this milestone worked when the viewer renders the same information in the terminal's colours and glyphs; when a transcript entry whose text is `<img src=x onerror=alert(1)>` displays those characters literally and creates no element; when a link written as `[click](javascript:alert(1))` renders as text; and when the Node-driven tests cover headings, nested lists, tables, code, and each of those injection cases.
...
In `crates/hel-cli/src/daemon.rs`, add:

    pub(crate) fn active_lifecycles(&self) -> Vec<RuntimeLifecycleView>;

a synchronous read of the existing `lifecycle` mutex, so the phone control loop can project operations on every publish without awaiting.
...
In `src/hel_database.rs` and `src/hel_database/schema.rs`, add:
    ...
    pub fn search_prompts_bounded(session_id: &str, bundle_id: &str, scope: HistoryScope,
                                  query: &str, limit: usize) -> Result<Vec<PromptHistoryEntry>>;
