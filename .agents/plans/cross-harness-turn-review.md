...
- Decision: reuse hel's second-opinion plumbing (profile staging, reviewer sidecar, relay transport, `ReviewerPane`, waterfall reviewer selection, SQLite `ReviewerDefaults`) rather than porting mj's host-side `ProgrammaticPool` spawn kernel. Review agents run inside the session's worker container like the existing reviewer.
  ...
  Date/Author: 2026-08-31 / jbellis + Fable.
- Decision (superseded 2026-08-31, see next two entries): port mj's prompts, lane roster, tier structure, supervisor loop, and verdict contract as directly as possible; drop Bifrost and use mj's raw-diff path.
  Rationale at the time: Bifrost looked optional in mj and hel does not ship it. Superseded because the maintainer judged the loss of the lane detectors too large, and inspection showed the premises were wrong: Bifrost is first-party (`@brokkai/bifrost`) and hel's container image is node-based.
  Date/Author: 2026-08-31 / jbellis + Fable.
- Decision: Bifrost is a required, first-class component of turn review — no enable flag, no degraded mode. It is built from source and baked into the container image at build time; lanes get the `core|slopcop` MCP toolset, the supervisor, validator, and quick reviewer get `core`; the `analyze_diff` pre-pass always runs; any Bifrost failure (missing from image, spawn failure, analysis error or timeout) resolves the review as Failed with an actionable message and never advances the baseline. `RawDiffSummary` is not a prompt path: it survives only as the worker's local diffstat calculator for `RepoDelta` protocol metadata. Pinning mirrors mj's discipline (mj pins an exact version — `DEFAULT_PINNED_VERSION`, `mjolnir/mj-core/src/bifrost.rs` — because a moving `latest` once shipped a performance regression to every review): hel pins an exact git revision, bumped only deliberately; there is no version configuration.
  Rationale: the maintainer rejects fallback paths ("fix the source of a problem"), and the lane detectors are the lanes' identity — the roster was distilled from slop-cop's code-review pack around these instruments. One path with instruments, failing loudly, beats two paths or one weak path. This also matches mj's default behavior (`bifrost_analysis: true`), minimizing divergence for a future re-merge.
  Date/Author: 2026-08-31 / jbellis + Fable.
- Decision: hel consumes Bifrost as a pinned crates.io release — `cargo install brokk-bifrost@<version> --locked` in a Containerfile builder stage, the full facade binary baked into the image — not via npm, not a git commit, and without trimming the policy engine out of the build. Runtime toolset gating is the isolation boundary: `--mcp core|slopcop` never exposes an `extended` (policy) tool. A follow-up upstream patch feature-gating `mcp_extended` and the `brokk-bifrost-policy` dependency (12 references) is available if image build time or binary size measurably hurts, but is deferred until measured; note that `bifrost-rql` would remain either way via `bifrost-runtime`'s dependency on it.
  Rationale: mj uses npm because it consumes released artifacts without the checkout; hel's image build can compile from source, which the maintainer prefers. Cutting policy at the Cargo level saves compile input (~119K lines of ~1.1M) but changes no runtime behavior, so per the measure-before-attributing rule it is not done speculatively.
  Date/Author: 2026-08-31 / jbellis + Fable.
...
Revision note (2026-08-31): initial version, authored from paired research passes over hel (`src/hel_second_opinion.rs`, `src/hel_worker_runtime/reviewer.rs`, `src/hel_chat/second_opinion.rs`, `src/hel_worker/*`, `src/hel_database/*`) and mjolnir (`mj-agents/src/discrete_review.rs`, `mj-core/src/orchestrator_contract.rs`, `mj-core/src/config.rs`); design decisions fixed in conversation with the maintainer on 2026-08-31 (synchronous lock, cumulative baselines, split-pane visibility, no new profile/config.toml surface).

Revision note (2026-08-31, second revision): reversed the initial "drop Bifrost" decision after the maintainer judged the lane detectors too valuable to lose and rejected fallback paths. Bifrost is now required and one-path: a pinned crates.io release (`cargo install brokk-bifrost@<version>`) baked into the container image — never npm; Bifrost is a first-party Rust workspace at `/home/jonathan/Projects/bifrost`, and its npm package is a distribution wrapper mj uses because it lacks the checkout. The policy engine stays in the build but is unreachable at runtime (`--mcp core|slopcop` never selects the `extended` toolset); a feature-gate trim upstream is deferred until build cost is measured. Intent-analyst failure now fails the review loudly instead of proceeding with an "unavailable" context, and intent runs concurrently with `analyze_diff` per the maintainer's rule: never serialize concurrent work to make a rare failure path cheaper. The per-harness MCP attachment plumbing moved from Milestone 4 to Milestone 2 because the quick tier's reviewer and validator now attach Bifrost navigation.
