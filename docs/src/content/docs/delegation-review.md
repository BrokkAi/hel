...
1. Six read-only specialist lanes review the turn's cumulative diff in parallel
   — cognitive complexity, structural duplication, error handling, dead code and
   unused abstraction, test signal, and comment intent. Lanes use Bifrost
   analyzer tools over MCP when the binary is present, and fall back to plain
   read-only inspection when it is not.
2. A single-shot synthesis pass on the primary's model vets the lane reports,
   discarding findings that are not caused by this turn, unsupported,
   speculative, or duplicated.
3. Surviving findings are injected as a corrective turn on the primary, framed
...
Lanes appear in the subagent status area as `review · {lane}` rows, and their
model usage is accounted to the review seat. Discrete review is toggled on the
Agents tab of `/mjconfig`.
