...
fn daemon_exits_when_its_store_is_migrated_underneath_it() {
    ...
    let config_directory = storage.path().join("config/hel");
    let data_directory = storage.path().join("data/hel");
    fs::create_dir_all(&config_directory).expect("create Hel config directory");
    fs::write(
...
environment = { PATH = "/hel-store-divergence-test-no-executables" }
...
"#,
    ...
    // No HEL_DAEMON_EXIT_WHEN_IDLE: an idle exit would end this process for a
    // reason that has nothing to do with the store.
    let mut daemon = ReapChild(Some(
        Command::new(env!("CARGO_BIN_EXE_hel"))
            .arg("daemon-run")
            .env("HEL_CONFIG_DIR", &config_directory)
    ...
    // What another Hel build's migration ladder does to a store this daemon
    // has open.
    let store = data_directory.join("hel.sqlite3");
