...
impl WorkflowEmitter {
    pub(crate) fn new(events: mpsc::UnboundedSender<UiEvent>) -> Self {
        Self {
    ...
    pub(crate) fn state(&self, id: WorkflowId) -> Option<WorkflowState> {
        self.inner
            .store
            .lock()
            .expect("workflow state lock poisoned")
...
#[derive(Debug, Clone)]
pub(crate) struct WorkflowActorContext {
...
impl WorkflowActorContext {
    pub(crate) fn actor_id(subagent_id: u64) -> WorkflowActorId {
        WorkflowActorId::Subagent(subagent_id)
    }

    pub(crate) fn started(&self, subagent_id: u64) {
    ...
    pub(crate) fn session_bound(&self, subagent_id: u64, session_id: String) {
        let _ = self.emitter.emit(WorkflowEvent::new(
            self.workflow_id,
            WorkflowTransition::ActorSessionBound {
                actor_id: Self::actor_id(subagent_id),
    ...
    pub(crate) fn resumed(&self, subagent_id: u64) {
        let _ = self.emitter.emit(WorkflowEvent::new(
            self.workflow_id,
    ...
    pub(crate) fn finished(&self, subagent_id: u64, outcome: SubagentOutcome) {
        let _ = self.emitter.emit(WorkflowEvent::new(
            self.workflow_id,
