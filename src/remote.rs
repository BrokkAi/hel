...
impl TrackerState {
    ...
    /// Mirror one subagent lifecycle event, the same way the TUI's status area
    /// does: `Started` and `Finished` also leave a permanent transcript line,
    /// `Activity` only refreshes the live row (no transcript churn).
...
fn handle_server_agent_event(
    ...
    if let UiEvent::Subagent(subagent_event) = event {
        // Status rows first: the mirror ignores everything that is not a
        // lifecycle event, so the arms below only handle the rest.
        ...
        match subagent_event {
            ...
            crate::event::SubagentEvent::SessionUpdate { update, .. } => {
                tracker.observe_actor_session_update(&update, "subagent", Some("subagent"));
            }
            crate::event::SubagentEvent::TerminalOutput { mut snapshot, .. } => {
                snapshot.terminal_id = namespace_remote_id(Some("subagent"), &snapshot.terminal_id);
