...
use crate::event::{
    InternalMessage, InternalMessageKind, SubagentEvent, SubagentOutcome, UiCommand, UiEvent,
    content_block_text,
};
...
impl Config {
    ...
    fn ensure_warm(&self, context: RunContext) {
        ...
        if slot.is_none() {
            *slot = Some(spawn_subagent_runtime(self, context, None));
        }
    }
...
impl McpHandler {
    ...
    async fn create_subagent(
        ...
        let admission = match self.controller.begin(root).await {
        ...
        self.note_handoff();
        if let Some(reports) = self.config.reports.as_ref() {
            reports.open();
        }
    ...
    async fn resume_subagent(
        ...
        let Some(run) = self.runs.take(subagent_id) else {
            return Err(McpError::invalid_params(
                unresolved_subagent_message(subagent_id),
                None,
            ));
        };
        if run.state == SubagentRunState::Running {
            self.runs.insert_running(subagent_id, run.control);
            return Ok(CallToolResult::error(vec![Content::text(
                still_running_message(subagent_id),
            )]));
        }
        if let Err(full) = self.controller.resume_retained(subagent_id).await {
            self.runs
                .reinstate_retained(subagent_id, run.control, self.config.max_parallel);
            return Ok(CallToolResult::error(vec![Content::text(full.message())]));
        }
        // Register before handing the worker the prompt: the worker can finish
        // and mark itself retained the instant it receives `Continue`.
        self.runs.insert_running(subagent_id, run.control.clone());
        if run
            .control
            .send(WorkerRequest::Continue { prompt })
            .is_err()
        {
            self.runs.take(subagent_id);
            self.controller.finish(subagent_id).await;
            return Ok(CallToolResult::error(vec![Content::text(
                worker_unavailable_message(subagent_id),
            )]));
        }
        self.note_handoff();
        if let Some(reports) = self.config.reports.as_ref() {
...
fn run_boxed(
    ...
    Box::pin(run(
        config, context, task, label, spec, ui_tx, lease, control_rx,
    ))
}
...
async fn run(
    ...
    let use_warm = spec.role.is_none();
    ...
    let WarmRuntime {
        ...
        spawn_subagent_runtime(&config, context.clone(), Some(termination.token.clone()))
    });
    if use_warm {
    ...
    let mut prompt_to_send = Some(format!("{SUBAGENT_PREAMBLE}{task}"));
    ...
    let mut result: Result<String> = 'session: loop {
        ...
        if termination.cause() == TerminationCause::None && cancel_respond.is_none() {
            ...
            let _ = ui_tx.send(UiEvent::Subagent(SubagentEvent::Status {
                subagent_id,
                message: "finished; session retained for resume".to_string(),
            }));
...
mod tests {
    ...
    async fn a_finished_run_reports_and_retains_its_session_for_resume() {
        ...
        for _ in 0..200 {
            if run.registry.retained_ids().contains(&run.subagent_id) {
                break;
            }
            tokio::task::yield_now().await;
        }
        assert_eq!(run.registry.retained_ids(), vec![run.subagent_id]);
        assert_eq!(
