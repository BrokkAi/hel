...
/// Wall-clock budget for Eitri's session-intent extraction.
...
async fn run(
    ...
    emit_internal(
        ...
        "Adversarial synthesis started. Verifying specialist reports and changed callables with Bifrost.",
    );
    let verdict = match run_supervisor(
        config,
        SupervisorEvidence {
            job: &job,
            reports: &reports,
...
/// Single-shot adversarial review on Thor's seat. Its failure is not fatal to
/// review value -- the orchestrator falls back to the single-prompt path -- so
/// it gets no model failover ladder of its own.
...
fn synthesis_prompt(
    ...
    format!(
        ...
        mcp_roots = mcp_roots_packet(repository_roots),
