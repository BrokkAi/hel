...
fn github_cli_wrapper_reads_each_live_token_and_clears_stale_environment() {
    ...
    let invoke = |expected_token: Option<&str>| {
        match expected_token {
            Some(token) => {
                crate::hel_credentials::write_github_token(&token_path, token.as_bytes()).unwrap();
            }
...
fn install_skills_replaces_the_session_tree_and_reports_the_new_state() {
    ...
    let archive = crate::hel_skills::collect_skills(HarnessKind::Codex, canonical.path()).unwrap();
...
async fn incompatible_protocol_cannot_read_or_mutate_credentials() {
    ...
    crate::hel_credentials::write_credential_file(endpoint.harness, &endpoint.marker, &original)
        .unwrap();
