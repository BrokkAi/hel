    ...
    pub fn from_config_state(config: &HelConfig, state: &HelState, revision: u64) -> Self {
        let sessions = state
            ...
            .map(|session| ViewerSession {
                ...
                checkpointed: session.checkpoint.is_some(),
