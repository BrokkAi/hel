...
use crate::hel_worker::RelayEvent;
...
pub(super) fn review_role_session_id(primary_session_id: &str, role: &str) -> String {
    crate::hel_review::host::role_session_id(primary_session_id, role)
}
...
mod tests {
    ...
    use crate::hel_second_opinion::{
        HARNESS_DEFAULT_VALUE, ReviewerDefaults, ReviewerProfileChoice,
    };
    use crate::hel_transcript::ChatRole;
    use crossterm::event::{KeyCode, KeyModifiers};

    fn plan_review() -> ElicitationRequest {
        crate::hel_acp::normalized_plan_review(
            "plan-review-1".into(),
    ...
    fn captured() -> CapturedProposal {
        ...
        let proposal = crate::hel_acp::plan_review_proposal(&request)
    ...
    fn choosing_the_second_opinion_never_answers_the_harness() {
        let crate::hel_elicitation::ElicitationFieldKind::SingleSelect { options, .. } =
        ...
        for steps in 0..options.len() {
            match answer_plan_review(steps) {
                ...
                ChatAction::RespondElicitation { response, .. } => {
                    let crate::hel_elicitation::ElicitationResponse::Accept { content } = response
                    else {
                        panic!("accepting the dialog produces an accept");
                    };
                    assert_ne!(
                        content.get("action"),
                        Some(&crate::hel_elicitation::ElicitationValue::String(
                            crate::hel_acp::PLAN_REVIEW_SECOND_OPINION.to_owned()
                        )),
    ...
    fn every_dialect_offers_the_second_opinion() {
        ...
        for value in [
            ...
            let request = crate::hel_acp::normalized_plan_review("plan-review-1".into(), &value);
            let crate::hel_elicitation::ElicitationFieldKind::SingleSelect { options, .. } =
            ...
            assert!(
                options
                    .iter()
                    .any(|option| option.value == crate::hel_acp::PLAN_REVIEW_SECOND_OPINION),
    ...
    fn a_harness_default_selection_is_stored_under_its_sentinel() {
        let selection = crate::hel_second_opinion::ReviewerSelection {
